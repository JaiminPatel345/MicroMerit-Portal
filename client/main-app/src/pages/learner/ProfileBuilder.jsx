...
import { Calendar, Check, CheckCircle, Lock, Upload, User } from 'lucide-react';
...
const ProfileBuilder = () => {
    ...
    const urlLoginMethod = searchParams.get('loginMethod');

    // Original state data (for email/phone signup)
    const { identifier, type, loginMethod: stateLoginMethod, tempToken: stateTempToken } = location.state || {};

    // Use URL login method or state login method
    const loginMethod = urlLoginMethod || stateLoginMethod;
    // Use URL temp token (OAuth) or state temp token (regular signup)
    const tempToken = urlTempToken || stateTempToken;
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (formData.gender) {
                ...
                submitData.append('gender',
                    genderMap[formData.gender] || formData.gender,
                );
            }
            ...
            console.log('Sending request to backend with tempToken...');
            ...
            if (response?.data?.success === true) {
                ...
                // Always use tokens from response (both OAuth and regular signup)
    ...
    return (
        <div
            className="min-h-screen bg-gradient-to-br from-blue-chill-50 to-white py-12 px-4 sm:px-6 lg:px-8">
            <div className="max-w-3xl mx-auto">
                <div className="text-center mb-8">
                    <h2 className="text-4xl font-bold text-gray-900 mb-2">Complete
                        Your Profile</h2>
                    <p className="text-gray-600">Tell us a bit about yourself to
                        get started</p>
                </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-6">
                        ...
                        <div className="flex justify-center mb-6">
                            <div className="relative">
                                <div
                                    className="w-32 h-32 rounded-full bg-blue-chill-100 flex items-center justify-center overflow-hidden border-4 border-white shadow-lg">
                                    {profilePreview ? (
                                        <img src={profilePreview} alt="Profile"
                                            className="w-full h-full object-cover" />
                                    ) : (
                                        <User
                                            className="w-16 h-16 text-blue-chill-600" />
                                    )}
                                </div>
                                <label
                                    htmlFor="profilePic"
                        ...
                        <div className="grid md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label
                                    className="block text-sm font-semibold text-gray-700 mb-2">Date
                                    of Birth</label>
                                <div className="relative">
                                    <div
                                    ...
                                    <DatePicker
                                        ...
                                        className={`block w-full pl-10 pr-3 py-3 border ${errors.dateOfBirth
                                            ? 'border-red-500'
                                            : 'border-gray-300'
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    <option value="prefer_not_to_say">Prefer not
                                        to say
                                    </option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                {passwordFocused && (
                                    <div
                                        className="absolute z-50 mt-2 p-3 bg-white rounded-lg border border-gray-200 shadow-xl w-full max-w-xs transform transition-all duration-200 ease-out">
                                        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Password
                                            Requirements</p>
                                        <ul className="space-y-2 text-sm">
                                            {[
                                                ...
                                                <li key={index}
                                                    className={`flex items-center ${item.met
                                                        ? 'text-green-600'
                                                        : 'text-gray-500'}`}>
                                                    {item.met ? (
                                                        <Check
                                                        ...
                                                        <div
                                                            className="w-4 h-4 mr-2 border-2 border-gray-300 rounded-full flex-shrink-0" />
                                                    )}
                                                    <span className={item.met
                                                        ? 'font-medium'
                                                        : ''}>{item.text}</span>
                                                </li>
                                            ))}
                                        ...
                                        <div
                                            className="absolute -top-2 left-6 w-4 h-4 bg-white border-t border-l border-gray-200 transform rotate-45"></div>
                                    </div>
                                )}
                            </div>
                            ...
                            <div>
                                ...
                                <div className="relative">
                                    ...
                                    <input
                                        ...
                                        onChange={handleInputChange}
                                        className={`block w-full pl-10 pr-3 py-3 border ${confirmPasswordError ||
                                ...
                                {(confirmPasswordError ||
                                    errors.confirmPassword) && (
                                        <p className="mt-1 text-sm text-red-600">{confirmPasswordError ||
                                    ...
                                    <label className="ml-3 text-sm">
                                        ...
                                        <p className="text-gray-600 mt-1">
                                            ...
                                            requirements. <a href="/terms"
                                                ...
                                                className="text-blue-chill-600 hover:text-blue-chill-800 underline">Terms</a>
                                        </p>
                                    </label>
                                </div>
                            </div>
                        ...
                        <div
                            ...
                            <div className="text-sm text-gray-700">
                                <p className="font-semibold text-green-900">Your
                                    data is secure</p>
                                <p>We use industry-standard encryption and
                                    blockchain technology to protect your
                                    credentials.</p>
                            </div>
                        </div>
